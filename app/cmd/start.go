...
import (
	...
	"github.com/goecology/oss-pic/app/pkg/bootstrap"
...
func startFn(cmd *cobra.Command, args []string) {
	bootstrap.Init()
	fmt.Println("gin.Config()------>", gin.Config())
	// 主服务器
	...
	fmt.Println("gin.Config().Muses.Server.Gin.Addr------>", gin.Config().Muses.Server.Gin.Addr)
	//fmt.Println("router.InitRouter()------>", router.InitRouter())
